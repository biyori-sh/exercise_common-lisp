...
(defun mrk-method (funcs curr-values interval-step &key (butcher-a *rk4-a-4s4o*) (butcher-b *rk4-b-4s4o*))
  (let ((k-list '()))
    (setf k-list (append k-list		;k1
                         (list (mult-scalar interval-step (multi-apply funcs curr-values)))))
    (dolist (stage butcher-a)		;k2--kn
      (setf k-list (append k-list
                           (list (mult-scalar interval-step (multi-apply funcs (mapcar #'+ curr-values (sum-mscalar stage k-list))))))))
  ...
  (labels ((pendulum-func (k)
           ...
           (mrk (funcs curr-values interval-step)
             (mrk-method funcs curr-values interval-step
                         :butcher-a *shanks-a-9s7o*
                         :butcher-b *shanks-b-9s7o*)))
...
(defun example-crk-method-mathieu ()
  ...
  (labels ((mathieu-func (delta epsilon bias alpha n)
    ...
    (plot-first-order-odes #'mcrk-method (mathieu-func 1.5 10.0 0.5 1.0 3) '(0.0 1.0 0.0) 1.e-2 2000 "cRK-mathieu")))
  ...
  (labels ((mathieu-func (delta epsilon bias alpha n)
           ...
           (mrk (funcs curr-values interval-step)
             (mrk-method funcs curr-values interval-step
                         :butcher-a *shanks-a-9s7o*
                         :butcher-b *shanks-b-9s7o*)))
    (plot-first-order-odes #'mrk (mathieu-func 1.5 10.0 0.5 1.0 3) '(0.0 1.0 0.0) 1.0e-2 2000 "RK-mathieu")))


(defun rk-method-mathieu (del eps alp interval steps)
  "d^2x/dt^2 = b - alpha*x^n - (d + e*cos(t))*x"
  (labels ((mathieu-func (delta epsilon alpha)
             (list (lambda (tau pos vel) (+ (* 0 tau pos vel) 1))
                   (lambda (tau pos vel) (+ (* 0 tau pos) vel))
                   (lambda (tau pos vel) (- (+ (* 0 vel) (* alpha epsilon (cos (* 2 tau))))
                                            (* pos (+ delta (* epsilon (cos (* 2 tau)))))))))
           (mrk (funcs curr-values interval-step)
             (mrk-method funcs curr-values interval-step
                         :butcher-a *shanks-a-9s7o*
                         :butcher-b *shanks-b-9s7o*)))
    (plot-first-order-odes #'mrk (mathieu-func del eps alp) '(0.0 0.0 1.0) interval steps "RK-mathieu-inf")))
